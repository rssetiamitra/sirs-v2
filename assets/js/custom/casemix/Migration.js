...
$(document).ready(function() {
  //initiate dataTables plugin
    oTable = $('#dynamic-table').DataTable({ 
      ...
      "ajax": {
          "url": "casemix/Migration/get_data",
          "type": "POST"
      },
      "columnDefs": [
        { 
...
function reset_table(){
    oTable.ajax.url('casemix/Migration/get_data').load();
...
function find_data_reload(result){
    ...
    oTable.ajax.url('casemix/Migration/get_data?num='+data.no_sep_mr+'&frmdt='+data.from_tgl_reg+'&todt='+data.to_tgl_reg+'&tipe='+data.tipe_ri_rj).load();
    $("html, body").animate({ scrollTop: "400px" });
...
function submit(no_registrasi){
    ...
    $.ajax({
        url: 'casemix/Migration/process',
        type: "post",
        data: {no_registrasi:no_registrasi,no_sep:sep,form_type:type},
        dataType: "json",
        beforeSend: function() {
          achtungShowLoader();  
        },
...
function getBillingDetail(noreg, type, field){
  preventDefault();
  $.getJSON("casemix/Migration/getRincianBilling/" + noreg + "/" + type + "/" +field, '', function (data) {
      response_data = data;
      html = '';
      html += '<div class="center"><p><b>RINCIAN BIAYA '+field+'</b></p></div>';
      //alert(response_data.html); return false;
      $('#detail-'+noreg).html(response_data.html);
  });
