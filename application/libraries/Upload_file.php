...
final Class upload_file {
    ...
    function doUpload($inputname, $path, $width='', $height='', $wdt_thumb='', $hgt_thumb='')
    {
        ...
        if(!empty($_FILES[$inputname]['name'])){
            ...
            if ($CI->upload->do_upload($inputname)){
                $data_upl = $CI->upload->data(); //print_r($data_upl);die;
                //Compress Image
    ...
    /*function doUpload($params, $inputname, $path)
    {
        ...
        $random = rand(1,99);
        $nama_file_unik = $random.preg_replace('/\s+/', '', $_FILES[''.$inputname.'']['name']);
        ...
        if(move_uploaded_file($_FILES[$inputname]["tmp_name"], $vfile_upload)){
            $datainsertattc = array(
                ...
                'flag' => $params['flag'],
                'attc_owner' => $CI->session->userdata('user')->fullname,
                'attc_name' => $_FILES[''.$inputname.'']['name'],
                'attc_path' => $nama_file_unik,
                'attc_fullpath' => $path.$nama_file_unik,
                'attc_type' => $type_file,
                'attc_size' => $_FILES[''.$inputname.'']['size'],
