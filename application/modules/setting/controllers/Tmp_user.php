...
class Tmp_user extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit user', 'Tmp_user/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add user', 'Tmp_user/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View user', 'Tmp_user/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        {                       
            ...
            if($id==0){
                ...
                $this->logs->save('tmp_user', $newId, 'insert new record', json_encode($dataexc));
            }else{
                ...
                $newId = $id;
                $this->logs->save('tmp_user', $newId, 'update record', json_encode($dataexc));
            }
    ...
    public function process_profile_user()
    {
        ...
        {                       
            ...
            if(isset($_FILES['images']['name'])){
                ...
                if( $id != 0 ){
                    ...
                    if ($profile->path_foto != NULL) {
                        unlink(PATH_IMG_DEFAULT.$profile->path_foto.'');
                        unlink(PATH_IMG_THUMB.$profile->path_foto.'');
                        unlink(PATH_IMG_RESIZED.$profile->path_foto.'');
                    }
                }

                $dataexc['path_foto'] = $this->upload_file->doUpload('images', PATH_IMG_DEFAULT , 100, 140);
            }
            ...
            if($id==0){
                ...
                $newId = $this->db->insert_id();
                $this->logs->save('tmp_user_profile', $newId, 'insert new record', json_encode($dataexc));
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL');
                $this->db->update('tmp_user_profile', $dataexc, array('user_id' => $id));
                $newId = $id;
                $this->logs->save('tmp_user_profile', $newId, 'update record', json_encode($dataexc));
            }
            if ($this->db->trans_status() === FALSE)
            {
                $this->db->trans_rollback();
    ...
    public function delete()
    {
        ...
        if($id!=null){
            if($this->Tmp_user->delete_by_id($toArray)){
                $this->logs->save('tmp_user', $id, 'delete record', '');
    ...
    public function form_update_profile()
    {
        ...
        $id=$this->session->userdata('user')->user_id;
        /*breadcrumbs for edit*/
        $this->breadcrumbs->push('Update Profile', 'Tmp_user/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*get value by id*/
        $data['value'] = $this->Tmp_user->get_by_id($id);
        /*initialize flag for form*/
        $data['flag'] = "update";
