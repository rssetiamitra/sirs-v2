...
class Tmp_mst_role extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit role', 'Tmp_mst_role/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add role', 'Tmp_mst_role/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View role', 'Tmp_mst_role/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        {                       
            ...
            if($id==0){
                ...
                $this->db->insert('tmp_mst_role', $dataexc);
                $newId = $this->db->insert_id();
                $this->logs->save('tmp_mst_role', $newId, 'insert new record', json_encode($dataexc));
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL');
                $this->db->update('tmp_mst_role', $dataexc, array('role_id' => $id));
                $newId = $id;
                $this->logs->save('tmp_mst_role', $newId, 'update record', json_encode($dataexc));
            }
    ...
    public function delete()
    {
        ...
        if($id!=null){
            if($this->Tmp_mst_role->delete_by_id($toArray)){
                $this->logs->save('tmp_mst_role', $id, 'delete record', '');
