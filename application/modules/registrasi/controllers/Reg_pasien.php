...
class Reg_pasien extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit registrasi', 'Reg_pasien/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Reg_pasien->get_by_id($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add registrasi', 'Reg_pasien/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Reg_pasien/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View registrasi', 'Reg_pasien/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Reg_pasien->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('registrasi/Reg_pasien','R',$row_list->function_id,2).'
                        '.$this->authuser->show_button('registrasi/Reg_pasien','U',$row_list->function_id,2).'
                        '.$this->authuser->show_button('registrasi/Reg_pasien','D',$row_list->function_id,2).'
                      </div>'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Reg_pasien->count_all(),
                        "recordsFiltered" => $this->Reg_pasien->count_filtered(),
                        "data" => $data,
                );
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Reg_pasien->delete_by_id($toArray)){
