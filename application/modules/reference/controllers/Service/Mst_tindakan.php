...
class Mst_tindakan extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'reference/Service/Mst_tindakan');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('Service/Mst_tindakan_model', 'Mst_tindakan');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Service/Mst_tindakan/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit Tindakan Pasien', 'Mst_tindakan/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Mst_tindakan->get_by_id($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add Tindakan Pasien', 'Mst_tindakan/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Service/Mst_tindakan/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View Tindakan Pasien', 'Mst_tindakan/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Mst_tindakan->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('reference/Service/Mst_tindakan','R',$row_list->icd10_id,2).'
                        '.$this->authuser->show_button('reference/Service/Mst_tindakan','U',$row_list->icd10_id,2).'
                        '.$this->authuser->show_button('reference/Service/Mst_tindakan','D',$row_list->icd10_id,2).'
                      </div>'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Mst_tindakan->count_all(),
                        "recordsFiltered" => $this->Mst_tindakan->count_filtered(),
                        "data" => $data,
                );
    ...
    public function process()
    {
        ...
        $val->set_rules('icd10_kode', 'Kode Tindakan Pasien', 'trim|required');
        $val->set_rules('icd10_nama', 'Nama Tindakan Pasien', 'trim|required');
        $val->set_rules('icd10_induk_kode', 'Kode Tindakan Pasien Induk', 'trim|required');
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Mst_tindakan->delete_by_id($toArray)){
