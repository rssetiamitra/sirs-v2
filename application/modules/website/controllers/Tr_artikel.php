...
class Tr_artikel extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'website/Tr_artikel');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            header("Refresh:0; url=".base_url()."login"); exit;
            //redirect(base_url().'login');exit;            
        }
        /*load model*/
        $this->load->model('Tr_artikel_model', 'Tr_artikel');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Tr_artikel/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit artikel', 'Tr_artikel/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Tr_artikel->get_by_id($id); 
            ...
            $this->breadcrumbs->push('Add artikel', 'Tr_artikel/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Tr_artikel/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View artikel', 'Tr_artikel/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Tr_artikel->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('website/Tr_artikel','R',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_artikel','U',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_artikel','D',$row_list->wp_id,2).'
                      </div>'; 
            $row[] = '<div class="center">'.$row_list->wp_id.'</div>';
            $row[] = ucfirst($row_list->wp_title);
            $row[] = $row_list->wp_author;
            $row[] = $row_list->wp_date;
            $row[] = substr($row_list->wp_content, 0,300);
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Tr_artikel->count_all(),
                        "recordsFiltered" => $this->Tr_artikel->count_filtered(),
                        "data" => $data,
                );
        ...
        {                       
           ...
           $dataexc = array(
            ...
            if($_FILES['images']['name'] != ''){
                ...
                if( $id != 0 ){
                    $tr_artikel = $this->Tr_artikel->get_by_id($id);
                    if (file_exists('uploaded/website/images/'.$tr_artikel->wp_images.'')) {
                        unlink('uploaded/website/images/'.$tr_artikel->wp_images.'');
                    }
                }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Tr_artikel->delete_by_id($toArray)){
                $this->logs->save('tr_artikel', $id, 'delete record', '');
