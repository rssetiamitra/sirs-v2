...
class Tr_file extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'website/Tr_file');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('Tr_file_model', 'Tr_file');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Tr_file/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit File', 'Tr_file/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Tr_file->get_by_id($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add File', 'Tr_file/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Tr_file/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View File', 'Tr_file/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Tr_file->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('website/Tr_file','R',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_file','U',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_file','D',$row_list->wp_id,2).'
                      </div>'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Tr_file->count_all(),
                        "recordsFiltered" => $this->Tr_file->count_filtered(),
                        "data" => $data,
                );
    ...
    public function process()
    {
        ...
        {                       
            ...
            $dataexc = array(
                ...
                'is_active' => $this->regex->_genRegex($val->set_value('wm_id'),'RGXAZ'),
            );

            if($_FILES['images']['name'] != ''){
                /*hapus dulu file yang lama*/
                if( $id != 0 ){
                    $tr_file = $this->Tr_file->get_by_id($id);
                    if (file_exists('uploaded/website/images/'.$tr_file->wp_images.'')) {
                        unlink('uploaded/website/images/'.$tr_file->wp_images.'');
                    }
                }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Tr_file->delete_by_id($toArray)){
                $this->logs->save('tr_file', $id, 'delete record', '');
