...
class Tr_berita extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'website/Tr_berita');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('Tr_berita_model', 'Tr_berita');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Tr_berita/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit berita', 'Tr_berita/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Tr_berita->get_by_id($id);
            $data['attachment'] = $this->upload_file->getUploadedFile($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add berita', 'Tr_berita/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Tr_berita/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View berita', 'Tr_berita/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Tr_berita->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('website/Tr_berita','R',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_berita','U',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_berita','D',$row_list->wp_id,2).'
                      </div>'; 
            $row[] = '<div class="center">'.$row_list->wp_id.'</div>';
            $row[] = ucfirst($row_list->wp_title);
            $row[] = $row_list->wp_author;
            $row[] = $row_list->wp_date;
            $row[] = '<p style="text-align:justify">'.substr(strip_tags($row_list->wp_content), 0, 300).'</p>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Tr_berita->count_all(),
                        "recordsFiltered" => $this->Tr_berita->count_filtered(),
                        "data" => $data,
                );
    ...
    public function process()
    {
        ...
        $val->set_rules('wp_category', 'Kategori', 'trim|required');
        $val->set_rules('wp_tags', 'Tags', 'trim|xss_clean');
        ...
        {                       
            ...
            $dataexc = array(
                ...
                'wp_tags' => $this->regex->_genRegex($val->set_value('wp_tags'),'RGXQSL'),
                'wp_category' => $this->regex->_genRegex($val->set_value('wp_category'),'RGXINT'),
            ...
            if($_FILES['images']['name'] != ''){
                ...
                if( $id != 0 ){
                    $tr_berita = $this->Tr_berita->get_by_id($id);
                    if (file_exists('uploaded/website/images/'.$tr_berita->wp_images.'')) {
                        unlink('uploaded/website/images/'.$tr_berita->wp_images.'');
                    }
                }

                $dataexc['wp_images'] = $this->upload_file->doUpload('images', 'uploaded/website/images/', 482, 320);
            }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Tr_berita->delete_by_id($toArray)){
                
                $this->logs->save('tr_berita', $id, 'delete record', '');
