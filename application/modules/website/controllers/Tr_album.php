...
class Tr_album extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'website/Tr_album');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('Tr_album_model', 'Tr_album');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Tr_album/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit album foto', 'Tr_album/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Tr_album->get_by_id($id);
            $data['attachment'] = $this->upload_file->getUploadedFile($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add album foto', 'Tr_album/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Tr_album/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View album foto', 'Tr_album/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {
        /*get data from model*/
        $list = $this->Tr_album->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center">
                        '.$this->authuser->show_button('website/Tr_album','R',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_album','U',$row_list->wp_id,2).'
                        '.$this->authuser->show_button('website/Tr_album','D',$row_list->wp_id,2).'
                      </div>'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Tr_album->count_all(),
                        "recordsFiltered" => $this->Tr_album->count_filtered(),
                        "data" => $data,
                );
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Tr_album->delete_by_id($toArray)){
                $this->logs->save('tr_album', $id, 'delete record', '');
