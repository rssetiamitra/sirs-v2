...
class Login_model extends CI_Model {
    ...
    public function get_hash_password($usr){
        $query = $this->db->select('tmp_user.*')
                          ->select('(SELECT GROUP_CONCAT(tmp_mst_role.`name` SEPARATOR "/") AS role FROM tmp_user_has_role LEFT JOIN tmp_mst_role ON tmp_mst_role.`role_id`=tmp_user_has_role.role_id WHERE user_id=tmp_user.user_id) as role')
    ...
    function find_menus($user_id, $parent) {
        ...
        $this->db->group_by('tmp_role_has_menu.menu_id');
