...
class Migration_model extends CI_Model {
    ...
    public function getDataLastMonth($month){
        $qry = "SELECT * FROM tc_registrasi WHERE YEAR(tgl_jam_keluar)=2018 AND MONTH(tgl_jam_keluar)=".$month." AND kode_perusahaan=120 ORDER BY no_sep ASC";
        return $this->sqlsrv->query($qry)->result();
    }
    ...
    public function getHasilLab($params, $kode_penunjang){
        //print_r($params);die;
        $this->sqlsrv->select("a.kode_tarif, a.nama_pemeriksaan, REPLACE(a.nama_tindakan, 'BPJS' , '') as nama_tindakan, a.hasil, a.standar_hasil_pria, a.standar_hasil_wanita, a.satuan, a.keterangan, a.detail_item_1, a.detail_item_2");
        $this->sqlsrv->from('pm_hasilpasien_v as a');
        $this->sqlsrv->where('a.kode_trans_pelayanan IN (SELECT kode_trans_pelayanan FROM tc_trans_pelayanan WHERE kode_penunjang='.$kode_penunjang.')');
        $this->sqlsrv->order_by('kode_tc_hasilpenunjang', 'ASC');
        return $this->sqlsrv->get()->result();
    }

    public function getNamaDokter($flag, $kode_pm){
        $this->sqlsrv->from('pm_tc_penunjang');
        $this->sqlsrv->join('mt_karyawan', 'mt_karyawan.kode_dokter=pm_tc_penunjang.dr_pengirim', 'left');
        $this->sqlsrv->where('pm_tc_penunjang.kode_penunjang', $kode_pm);
        $exc = $this->sqlsrv->get()->row();
        //print_r($this->sqlsrv->last_query());die;
