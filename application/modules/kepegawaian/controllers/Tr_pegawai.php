...
class Tr_pegawai extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'kepegawaian/tr_pegawai');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('kepegawaian/tr_pegawai_model', 'tr_pegawai');
        /*enable profiler*/
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit Data Pegawai', 'tr_pegawai/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add Data Pegawai', 'tr_pegawai/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View Data Pegawai', 'tr_pegawai/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*define data variabel*/
        $data['value'] = $this->tr_pegawai->get_by_id($id);
        $data['title'] = "Tr_pegawai";
        $data['flag'] = "read";
        $data['breadcrumbs'] = $this->breadcrumbs->show();
        /*load form view*/
        $this->load->view('tr_pegawai/form', $data);
    ...
    {
        /*get data from model*/
        $list = $this->tr_pegawai->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center"><div class="hidden-sm hidden-xs action-buttons">
                        '.$this->authuser->show_button('kepegawaian/tr_pegawai','R',$row_list->pg_id,2).'
                        '.$this->authuser->show_button('kepegawaian/tr_pegawai','U',$row_list->pg_id,2).'
                        '.$this->authuser->show_button('kepegawaian/tr_pegawai','D',$row_list->pg_id,2).'
                      </div>
                      <div class="hidden-md hidden-lg">
                        <div class="inline pos-rel">
                            ...
                            <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                <li>'.$this->authuser->show_button('kepegawaian/tr_pegawai','R',$row_list->pg_id,4).'</li>
                                <li>'.$this->authuser->show_button('kepegawaian/tr_pegawai','U',$row_list->pg_id,4).'</li>
                                <li>'.$this->authuser->show_button('kepegawaian/tr_pegawai','D',$row_list->pg_id,4).'</li>
                            </ul>
                        </div>
                    </div></div>';        
            $data[] = $row;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->tr_pegawai->count_all(),
                        "recordsFiltered" => $this->tr_pegawai->count_filtered(),
                        "data" => $data,
                );
    ...
    public function process()
    {
        ...
        {                       
            ...
            if($id==0){
                ...
                $this->db->insert('tr_pegawai', $dataexc);
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = $this->session->userdata('user')->fullname;
                $this->db->update('tr_pegawai', $dataexc, array('pg_id' => $id));
            }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->tr_pegawai->delete_by_id($toArray)){
...
/* Location: ./application/modules/tr_pegawai/controllers/tr_pegawai.php */
